...
var _Sources = (() => {
  ...
  // src/ReaperScans/ReaperScans.ts
  var ReaperScans_exports = {};
  __export(ReaperScans_exports, {
    ReaperScans: () => ReaperScans,
    ReaperScansInfo: () => ReaperScansInfo
  });
  var import_types3 = __toESM(require_lib());

  // src/ReaperScans/parser.ts
  ...
  var Parser3 = class {
    constructor() {
      ...
      this.REAPERSCANS_CDN = "https://media.reaperscans.com/file/4SRBHm";
      // https://domain.tld/file/<bucket>/<file>
      this.ID_SEP = "|#|";
    }
    //LINK - MangaDetails
  ...
  var ReaperScans = class {
    ...
    async getChapters(mangaId) {
      ...
      const baseRequestUrl = `${this.apiUrl}/chapters/${mangaId.split(this.parser.ID_SEP)[0]}`;
      ...
      this.checkResponseError(response);
      const json = JSON.parse(response.data ?? "[]");
      let chapterList = json.data;
      if (chapterList.length < (json.meta?.total ?? 0)) {
        const pages = Math.ceil(json.meta?.total / 30);
        for (let i = 2; i <= pages; i++) {
          ...
          const queryString2 = this.parser.joinParams(params2);
          const constructedURL2 = baseRequestUrl + "?query=" + queryString2;
    ...
    async getHomePageSections(sectionCallback) {
      const dataDaily = await this.parser.getMangaItems(`${this.apiUrl}/trending?type=daily`, this).then((data2) => {
        return data2.filter((item) => item.badge == "Manhwa");
      });
      const dataWeekly = await this.parser.getMangaItems(`${this.apiUrl}/trending?type=weekly`, this).then((data2) => {
        return data2.filter((item) => item.badge == "Manhwa");
      });
      const params = {
        series_type: "Comic",
...
`;
  ...
  return __toCommonJS(ReaperScans_exports);
})();
