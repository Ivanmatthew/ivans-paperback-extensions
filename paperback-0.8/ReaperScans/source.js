...
var _Sources = (() => {
  ...
  var ReaperScans = class {
    ...
    async getHomePageSections(sectionCallback) {
      const dataDaily = await this.parser.getMangaItems(`${this.apiUrl}/trending?type=daily`, this).then((data) => {
        return data.filter((item) => item.badge == "Manhwa");
      });
      const dataWeekly = await this.parser.getMangaItems(`${this.apiUrl}/trending?type=weekly`, this).then((data) => {
        return data.filter((item) => item.badge == "Manhwa");
      });
      const params = {
        series_type: "Comic",
