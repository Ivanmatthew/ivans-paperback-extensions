...
var _Sources = (() => {
  ...
  // src/AsuraScans/utils/PropsReviver.ts
  function createPropsReviver() {
    const propTypes = {
    ...
    const reviveTuple = (raw) => {
      ...
      return type in propTypes ? valueFn(value) : void 0;
    };
  ...
  // src/AsuraScans/AsuraScansParser.ts
  function retrieveProps($2, selector) {
    const rawProps = $2(selector).attr("props");
    if (!rawProps) {
  ...
  var HOME_SECTIONS = [
    {
      ...
      parser: ($2) => {
        ...
        const items = [];
        const props = retrieveProps($2, selector);
        if (!props) {
        ...
        for (const item of props.items) {
          items.push(
            App.createPartialSourceManga({
              mangaId: item.slug,
              title: (0, import_html_entities.decode)(item.title),
    ...
    },
    {
      ...
      parser: ($2) => {
        ...
        for (const chapter of comics.values()) {
          const isEarlyAccess = chapter.early_access_until ? new Date(chapter.early_access_until) > /* @__PURE__ */ new Date() : chapter.is_premium;
          items.push(
            App.createPartialSourceManga({
              mangaId: chapter.comic_slug,
              title: (0, import_html_entities.decode)(chapter.comic_name),
              subtitle: `${isEarlyAccess ? "[Early Access] " : ""}Ch. ${chapter.number}${chapter.title ? ` - ${(0, import_html_entities.decode)(chapter.title)}` : ""}`,
  ...
  var parseMangaDetails = async ($2, mangaId) => {
    ...
    const image = $2("div[class*='z-0'] img[class='w-full h-full object-cover']").attr(
      "src"
    ) ?? "";
    const rawStatus = $2(".text-base.capitalize").text().trim();
    ...
    const statusTagSection = statusTagInfo ? App.createTagSection({
    ...
    const description = $2("#description-text").text().trim();
    const genres = $2("div[class='hidden lg:flex max-w-full gap-2 flex-wrap'] a").map((i, el) => {
    ...
    const rawType = $2(".text-base.uppercase").text().trim();
    ...
    return App.createSourceManga({
      ...
      mangaInfo: App.createMangaInfo({
        titles,
        image,
        status: statusTagInfo?.label ?? rawStatus.charAt(0).toUpperCase() + rawStatus.slice(1),
    ...
    for (const chapter of props.chapters) {
      ...
      chapters.push(
        App.createChapter({
          ...
          name: `Ch. ${chapter.number}${chapter.title ? ` - ${(0, import_html_entities.decode)(chapter.title)}` : ""}`,
  ...
  // src/AsuraScans/utils/URLBuilder.ts
  ...
  // src/AsuraScans/AsuraScans.ts
  var AS_DOMAIN_NAME = "asurascans.com";
  var AS_DOMAIN = `https://${AS_DOMAIN_NAME}`;
  ...
  var AsuraScans = class _AsuraScans {
    ...
    async getSearchResults(query, metadata) {
      ...
      return App.createPagedResults({
        results: seriesData.data.map((series) => {
          const sortedLatestChapters = series.latest_chapters.sort(
            (a, b) => new Date(b.published_at).getTime() - new Date(a.published_at).getTime()
          );
          const latestChapter = sortedLatestChapters.length > 0 ? sortedLatestChapters[0] : void 0;
          const isEarlyAccess = latestChapter ? latestChapter.early_access_until ? new Date(latestChapter.early_access_until) > /* @__PURE__ */ new Date() : latestChapter.is_premium : false;
          return App.createPartialSourceManga({
            mangaId: series.slug,
            title: series.title,
            image: series.cover,
            subtitle: latestChapter ? `${isEarlyAccess ? "[Early Access] " : ""}Ch. ${latestChapter.number}${latestChapter.title ? ` - ${(0, import_html_entities2.decode)(latestChapter.title)}` : ""}` : "No chapters"
          });
        }).sort((a, b) => {
          if (query.title) {
