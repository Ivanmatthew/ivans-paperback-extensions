  ...
  var MangaK = class {
    constructor() {
      ...
      this.sections = {
        ...
        trending: {
          extractor: (data) => {
            return data.trendingItems.map(
              (item) => App.createPartialSourceManga({
                mangaId: item.slug,
                title: item.name,
                image: item.cover,
                subtitle: `\u2B50 ${item.displayRating} \u{1F525} ${item.displayViews}`
              })
            );
          },
          section: App.createHomeSection({
      ...
      this.SEARCH_FIELDS = SEARCH_FIELDS.map(App.createSearchField);
      this.TAG_SECTIONS = TAG_SECTIONS.map(
        (section) => App.createTagSection({
          id: section.id,
          label: section.label,
      ...
      this.requestManager = App.createRequestManager({
        ...
        interceptor: {
          ...
          interceptResponse: async (response) => {
            if (response.status >= 400 && response.status < 500 || response.data && "success" in JSON.parse(response.data) && JSON.parse(response.data).success === false) {
    ...
    async fetchBuildId() {
      return await this.requestManager.schedule(
        App.createRequest({
          url: `${API_URL}/version`,
          method: "GET"
        }),
    ...
    async getHomePageSections(sectionCallback) {
      ...
      await this.requestManager.schedule(request, 1).then((res) => {
        const data = JSON.parse(res.data ?? "{}");
        Object.values(this.sections).forEach((sectionObj) => {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const urlBuilder = new URLBuilder(API_DOMAIN_URL).addPathComponent("titles").addPathComponent("search").addQueryParameter("page", page.toString()).addQueryParameter("limit", "24").addQueryParameter("sort", sectionObj.viewMore.sort);
      const response = await this.requestManager.schedule(
        App.createRequest({
          method: "GET",
      ...
      const results = data.data.items.map(
        (item) => App.createPartialSourceManga({
          mangaId: item.slug,
          title: item.name,
          image: item.cover,
      ...
      return App.createPagedResults({
        results,
        metadata: pagination.has_next ? { ...pagination, page: pagination.page + 1 } : void 0
      });
    }
    async getMangaDetails(mangaId) {
      ...
      if (manga.formats.length > 0) {
        tagSections.push(
          App.createTagSection({
            id: "formats",
            label: "Formats",
            tags: manga.formats.map(
              (format2) => App.createTag({
                id: format2.slug,
                label: format2.name
              })
            )
          ...
          App.createTagSection({
            ...
            tags: [
              App.createTag({
                ...
                label: manga.contentRating.charAt(0).toUpperCase() + manga.contentRating.slice(1)
              })
            ]
          })
        );
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          titles,
          image: manga.cover,
          author: manga.authors.length > 0 ? manga.authors.map((author) => author.name).join(", ") : "Unknown",
          artist: manga.artists.length > 0 ? manga.artists.map((artist) => artist.name).join(", ") : "Unknown",
          desc: manga.summary,
          status: manga.status,
          tags: tagSections,
          covers: [manga.cover]
    ...
    async getSearchResults(query, metadata) {
      const page = metadata?.page ?? 1;
      const urlBuilder = new URLBuilder(API_DOMAIN_URL).addPathComponent("titles").addPathComponent("search").addQueryParameter("page", page.toString()).addQueryParameter("limit", "24");
      ...
      const getTagIdsForSection = (tags, sectionId) => {
        const sectionTagIds = tagSectionsById[sectionId];
        if (!sectionTagIds) {
          return [];
        }
        return tags.map((tag) => tag.id).filter((tagId) => sectionTagIds.has(tagId));
      };
      const getGenreTagIds = (tags) => {
        return tags.map((tag) => tag.id).filter((tagId) => !knownStaticTagIds.has(tagId));
      };
      ...
      if (minChapterCount !== void 0 && minChapterCount !== "") {
      ...
      const results = fuse.search(title ?? "").map((result) => App.createPartialSourceManga(result.item));
      return App.createPagedResults({
        results,
    ...
    async getSearchTags() {
      const tagSections = [...this.TAG_SECTIONS];
      ...
      genresData.data.items.forEach((genre) => {
        if (tagSections.some(
          (section) => section.tags.some((tag) => tag.id === genre.slug)
        )) {
          return;
        }
        genreTags.push(
