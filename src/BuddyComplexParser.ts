...
export class BuddyComplexParser {
    parseMangaDetails($: CheerioAPI, mangaId: string): SourceManga {
        ...
        const altTitles = $('div.name.box > h2')
            .text()
            .split(/, |; |\| |\/ /)
        ...
        const rawStatus = $('p:contains(Status) > a', 'div.detail')
            .first()
            .text()
            .trim()
        let status: string
    ...
    parseChapterList($: CheerioAPI, mangaId: string): Chapter[] {
        ...
        for (const chapter of $('li', 'ul.chapter-list').toArray()) {
            ...
            const byNumber = (id.split('-').pop() ?? '').match(
                /(\d+)(?:[-.]\d+)?/
            )
            const chapterNumberRegex =
                byChapter && byChapter[1]
                    ? byChapter[1]
                    : byNumber
                    ? byNumber[0]
                    : '0'
    ...
    getDeobfuscatedImageUrlBase($: CheerioAPI): [string, string] {
        ...
        if (!resPathMatch || resPathMatch.length < 2) {
            throw new Error(
                'Deobfuscation script does not contain resPath! Please report this issue.'
            )
        }
        ...
        return [`https://${sub}.${dm}.${dot}`, resPath]
    }
        ...
        for (const section of sections) {
            ...
            if (section.id == 'hot_updates') {
                ...
                for (const manga of $(
                    ...
                    const subtitle = $('span.latest-chapter', manga)
                        .text()
                        .trim()
    ...
    isLastPage = ($: CheerioAPI): boolean => {
        ...
        const currentPage = Number(
            $('a.link.active', 'div.paginator').text().trim()
        )
    ...
    protected getImageSrc(imageObj: Cheerio<Element> | undefined): string {
    ...
    protected parseDate = (date: string): Date => {
        ...
        if (extractedNumber == '') {
            ...
            number = isNaN(Number(extractedNumber))
                ? 0
